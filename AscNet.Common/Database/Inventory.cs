    ...
    public class Inventory
    {
        ...
        public static long GetMaxCount(ItemTable? itemTable)
        {
            if (itemTable is null)
                return GlobalItemMaxCount;

            if (itemTable.ItemType == (int)ItemType.Money)
                return ClampMoneyItemMaxCount(itemTable.MaxCount ?? MoneyItemMaxCount);

            if (IsNonTimedResearchTicket(itemTable))
                return MoneyItemMaxCount;

            if (itemTable.ItemType != (int)ItemType.Gift && itemTable.MaxCount > GlobalItemMaxCount)
                return ClampMoneyItemMaxCount(itemTable.MaxCount ?? MoneyItemMaxCount);

            return Math.Min(itemTable.MaxCount ?? GlobalItemMaxCount, GlobalItemMaxCount);
        }

        private static long ClampMoneyItemMaxCount(long maxCount)
        {
            return Math.Min(maxCount, MoneyItemMaxCount);
        }

        private static bool IsNonTimedResearchTicket(ItemTable itemTable)
        {
            return itemTable.ItemType == (int)ItemType.ResearchTicket
                && (itemTable.TimelinessType ?? 0) == 0;
        }
