        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            foreach (var table in context.AdditionalFiles.Where(x => x.Path.EndsWith(".tsv")))
            {
                try
                {
                    ...
                    string normalizedPath = table.Path.Replace("\\", "/");
                    string sourceRoot = normalizedPath.IndexOf("/Schemas/", StringComparison.Ordinal) >= 0
                        ? "/Schemas/"
                        : "/Resources/";
                    int sourceRootIndex = normalizedPath.IndexOf(sourceRoot, StringComparison.Ordinal);
                    if (sourceRootIndex < 0)
                        throw new FormatException($"Table path is outside the supported schema/resource roots: {table.Path}");
                    string runtimePath = normalizedPath.Substring(sourceRootIndex + sourceRoot.Length);
...
{{
    ...
    {{
        public static string File => ""{runtimePath}"";
